...
export function GlobalPresence() {
  ...
  return (
    <section id="global-presence" className="py-24 bg-slate-50 border-b border-slate-200/50 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none -z-10 bg-gradient-to-b from-white via-slate-50 to-white" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          ...
          <div className="lg:col-span-12 xl:col-span-6">
            <div className="relative rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm flex flex-col justify-between min-h-[440px]">
              ...
              <div className="relative h-56 w-full bg-slate-50 border border-slate-200/60 rounded-2xl overflow-hidden flex items-center justify-center">
                <div 
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-40">
                  {/* Lines between Pakistan (72%, 52%), UK (46%, 28%), Dubai (61%, 44%), NY (25%, 32%) */}
                  <path d="M 180 72 Q 220 50 330 63 Z" fill="none" stroke="#326E45" strokeWidth="1" strokeDasharray="3,3" />
                  <path d="M 330 63 Q 390 85 450 110 Z" fill="none" stroke="#326E45" strokeWidth="1" strokeDasharray="3,3" />
                  <path d="M 450 110 Q 420 125 500 122 Z" fill="none" stroke="#326E45" strokeWidth="1" strokeDasharray="3,3" />
                </svg>
                ...
                <button 
                  ...
                  className="absolute left-[25%] top-[25%] group cursor-pointer"
                >
                  <span className="absolute inset-0 w-6 h-6 rounded-full bg-emerald-500/10 -translate-x-2 -translate-y-2 animate-ping" />
                  <span className={`block w-3.5 h-3.5 rounded-full border-2 border-white shadow-xs transition-all ${
                    hoveredMarket === 2 ? 'bg-[#326E45]' : 'bg-slate-700'
                  }`} />
                  <span className="absolute top-4 left-1/2 -translate-x-1/2 bg-slate-900 text-white font-mono text-[7px] font-bold px-1 py-0.5 rounded shadow-xs whitespace-nowrap">NY</span>
                </button>
                ...
                <button 
                  ...
                  onMouseLeave={() => setHoveredMarket(null)}
                  className="absolute left-[46%] top-[20%] group cursor-pointer"
                >
                  <span className="absolute inset-0 w-6 h-6 rounded-full bg-emerald-500/10 -translate-x-2 -translate-y-2 animate-ping" />
                  <span className={`block w-3.5 h-3.5 rounded-full border-2 border-white shadow-xs transition-all ${
                    hoveredMarket === 0 ? 'bg-[#326E45]' : 'bg-slate-700'
                  }`} />
                  <span className="absolute top-4 left-1/2 -translate-x-1/2 bg-slate-900 text-white font-mono text-[7px] font-bold px-1 py-0.5 rounded shadow-xs whitespace-nowrap">LDN</span>
                </button>
                ...
                <button 
                  ...
                  onMouseLeave={() => setHoveredMarket(null)}
                  className="absolute left-[62%] top-[38%] group cursor-pointer"
                >
                  <span className="absolute inset-0 w-6 h-6 rounded-full bg-emerald-500/10 -translate-x-2 -translate-y-2 animate-ping" />
                  <span className={`block w-3.5 h-3.5 rounded-full border-2 border-white shadow-xs transition-all ${
                    hoveredMarket === 1 ? 'bg-[#326E45]' : 'bg-slate-700'
                  }`} />
                  <span className="absolute top-4 left-1/2 -translate-x-1/2 bg-slate-900 text-white font-mono text-[7px] font-bold px-1 py-0.5 rounded shadow-xs whitespace-nowrap">DXB</span>
                </button>
                ...
                <button 
                  ...
                  onMouseLeave={() => setHoveredMarket(null)}
                  className="absolute left-[72%] top-[39%] group cursor-pointer"
                >
                  <span className="absolute inset-0 w-6 h-6 rounded-full bg-emerald-500/15 -translate-x-2 -translate-y-2 animate-ping" fill="currentColor" />
                  <span className={`block w-4 h-4 rounded-full border-2 border-white shadow-xs transition-all ${
                    hoveredMarket === 3 ? 'bg-[#326E45] scale-125 shadow-md' : 'bg-[#326E45]'
                  }`} />
                  <span className="absolute top-5 left-1/2 -translate-x-1/2 bg-slate-900 text-white font-mono text-[8px] font-black px-1.5 py-0.5 rounded shadow-xs whitespace-nowrap">PK R&D Center</span>
                </button>

                <div className="absolute bottom-2 right-3 font-mono text-[8px] text-slate-400">
